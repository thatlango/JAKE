...
app.get('/fetch-stats',async(req,res)=>{try{const url=await assertExternalUrl(req.query.url),r=await fetch(url.toString(),{headers:{'User-Agent':'JakeOS/6.2'},redirect:'error',signal:AbortSignal.timeout(8000)}),text=await r.text();if((r.headers.get('content-type')||'').includes('application/json')){try{return res.json(JSON.parse(text));}catch{}}res.type('text/plain').send(text.slice(0,50000));}catch(e){res.status(e.status||502).json({error:e.message||'Could not fetch URL'});}});

app.post('/groq/transcribe',async(req,res)=>{const groq=String(process.env.GROQ_API_KEY||'').trim(),openai=String(process.env.OPENAI_API_KEY||'').trim();if(!groq&&!openai)return res.status(503).json({error:'Configure GROQ_API_KEY or OPENAI_API_KEY on the server'});const{audio,mimeType,filename}=req.body;if(!audio)return res.status(400).json({error:'No audio data'});try{const buffer=Buffer.from(audio,'base64');if(buffer.length>10*1024*1024)return res.status(413).json({error:'Audio payload is too large'});const formData=new FormData();formData.append('file',new Blob([buffer],{type:mimeType||'audio/webm'}),filename||'memo.webm');const provider=groq?'groq':'openai';formData.append('model',groq?'whisper-large-v3':String(process.env.OPENAI_TRANSCRIBE_MODEL||'gpt-4o-mini-transcribe'));formData.append('response_format','json');const endpoint=groq?'https://api.groq.com/openai/v1/audio/transcriptions':'https://api.openai.com/v1/audio/transcriptions';const key=groq||openai;const r=await fetch(endpoint,{method:'POST',headers:{Authorization:`Bearer ${key}`},body:formData,signal:AbortSignal.timeout(90000)});const d=await r.json().catch(()=>({}));if(!r.ok)return res.status(r.status).json({error:d.error?.message||'Transcription failed'});res.json({...d,provider});}catch(e){res.status(500).json({error:e.message});}});
