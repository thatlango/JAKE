...
function buildWhere(opts,values){ const clauses=[]; for(const [key,op] of [['eq','='],['neq','<>'],['gte','>='],['lte','<='],['gt','>'],['lt','<']]) for(const [col,val] of Object.entries(opts[key]||{})){values.push(dbValue(val));clauses.push(`${ident(col)} ${op} $${values.length}`);} if(opts.filter){const map={eq:'=',neq:'<>',gt:'>',gte:'>=',lt:'<',lte:'<=',like:'LIKE',ilike:'ILIKE'};const op=map[opts.filter.op];if(!op)throw new Error('Unsupported filter op');values.push(dbValue(opts.filter.val));clauses.push(`${ident(opts.filter.col)} ${op} $${values.length}`);} return clauses.length?` WHERE ${clauses.join(' AND ')}`:''; }
