...
export default function Agents({openAI}){
  ...
  const resolveDecision=async(item,approved)=>{
    ...
    try{
      ...
      await load();
    }catch(err){setError(err.message||'Decision could not be resolved.');}
    setBusy('');
  };
  ...
  const approvals=state.decisions.filter(item=>item.metadata?.type==='tool_approval');
  const normalDecisions=state.decisions.filter(item=>item.metadata?.type!=='tool_approval');
