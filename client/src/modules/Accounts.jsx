...
export default function Accounts(){
  ...
  useEffect(()=>{load(filters);},[]); // initial URL filters only
  ...
  return <div className="module accounts-page">
    <PageHeader eyebrow="System · Tuku Core" title="Accounts" subtitle="Canonical identities, access and recent activity across the Tuku estate." actions={<Button variant="secondary" icon="refresh" onClick={()=>load(filters)}>Refresh</Button>}/>
    {state.error&&<StateBanner tone="danger" title="Account directory unavailable">{state.error}</StateBanner>}
    <div className="px-metrics accounts-metrics"><Metric icon="users" label="Total accounts" value={state.loading&&!state.data?'—':totals.totalAccounts??0} helper="Canonical Tuku identities"/><Metric icon="check" label="Active / 7d" value={state.loading&&!state.data?'—':totals.active7d??0} helper="Unique recently active accounts" tone="success"/><Metric icon="plus" label="New / 30d" value={state.loading&&!state.data?'—':totals.new30d??0} helper="Recently created"/><Metric icon="warning" label="Suspended" value={state.loading&&!state.data?'—':totals.suspended??0} helper="Access currently blocked" tone={Number(totals.suspended)>0?'warning':'neutral'}/></div>
