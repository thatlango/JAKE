  ...
  return <div className="ops-page">
    ...
    <section className="ops-section">
      ...
      <div className="ops-table">
        ...
        {(data?.services||[]).map(s=>{const ok=Number(s.last_status)>=200&&Number(s.last_status)<400&&Number(s.consecutive_failures||0)===0;const tls=daysUntil(s.tls_expires_at);return <div className="ops-tr" key={s.id}><span><strong>{s.name}</strong><small>{s.product}</small></span><span><StatusDot state={ok?'healthy':Number(s.last_status)>=400&&Number(s.last_status)<500?'attention':s.last_checked_at?'critical':'unknown'}/>{s.last_status||'—'}{Number(s.consecutive_failures||0)>0&&<small>{s.consecutive_failures} failures</small>}</span><span>{fmtMs(s.last_latency_ms)}</span><span>{tls==null?'—':`${tls}d`}<small>{s.tls_expires_at?fmtDate(s.tls_expires_at):''}</small></span><span>{age(s.last_checked_at)}</span></div>;})}
      </div>
    </section>
